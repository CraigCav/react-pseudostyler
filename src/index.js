    ...
    for (let i = 0; i < count; i++) {
      ...
      if (sheet.href) {
        ...
        if (
          ...
          this.addCSS(sheet.ownerNode.firstChild.wholeText);
        }
      }
    }
  }
...
const ReactPseudoStyler = ({ target, selector, children }) => {
  const ref = useRef();
  ...
  useEffect(() => {
    async function togglePseudoStyles() {
      ...
      const boundQueries = Object.entries(queries).reduce(
        (res, [key, value]) => ({
          ...res,
          [key]: value.bind(null, ref.current)
        }),
        {}
      );

      let elem = target(boundQueries);

      if (!elem) return;

      styler.toggleStyle(elem, selector);

      for (; elem && elem !== ref.current; elem = elem.parentNode)
        styler.toggleStyle(elem, ':hover');
    }
...
ReactPseudoStyler.displayName = 'PseudoStyler';

export default ReactPseudoStyler;
